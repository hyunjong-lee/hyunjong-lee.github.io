...
국가별로 서비스를 적용하기 위해서는 접속한 클라이언트가 어느 국가인지 알아야 하는데 `GeoIP` 모듈을 통해 NGINX 내에서 국가 탐지가 가능하다. 자세한 사항은 책을 보면 되는데 간단히 설명하면 국가별로 classless iner-domain routing (CIDR)에 대한 정보가 있고 클라이언트의 IP가 CIDR 중 어떤 국가에 매칭되는지 확인하는 식이다.
...
최근엔 [superset](https://github.com/apache/superset)에 자체적인 인증 권한 부여가 필요해서 아래와 같은 코드도 구현했었다.
...
인증을 직접하게 되면 특정 웹서버에서 인증을 진행하면 다른 웹서버에선 해당 사실을 알기 위해 전역 캐싱을 하는 등 공유 작업이 필요하다. 즉, JWT 인증 자체를 구현했다고 끝이 아니라 통합테스트까지 진행해야 한다는 의미이다.

이 책에 소개된 내용에 따르면 NGINX Plus에 일반적인 백엔드 요구사항 수준에 필요한 기능은 모두 있는 것으로 보인다. 따라서 특수한 경우가 아니라면 인증이라는 바퀴를 직접 만드는 것이 아니라 NGINX의 인증을 사용하는 것이 좋은 판단일 것이다.
